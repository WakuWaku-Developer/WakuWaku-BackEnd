package dev.backend.wakuwaku.member;

import com.fasterxml.jackson.databind.ObjectMapper;
import dev.backend.wakuwaku.domain.member.controller.member.MemberController;
...
public class MemberControllerTest {
    ...
    public void testRegister() throws Exception {
        ...
        registerRequest.setPassword("testMemberPassword");


        // save 엔드포인트에 post요청 보냄
        mockMvc.perform(MockMvcRequestBuilders.post("/member/save")
                        // json 문자열로 변환
    ...
    public void testUpdate() throws Exception {
        ...
        // memberService.findById() 메서드 호출 -> 반환값 설정
        when(memberService.update(memberId, memberUpdateRequest)).thenReturn(memberId);


        // PUT 요청으로 해당 엔드포인트에 회원 정보 업데이트 요청을 보냄
        mockMvc.perform(MockMvcRequestBuilders.put("/member/{id}", memberId)
                        .contentType(MediaType.APPLICATION_JSON)
